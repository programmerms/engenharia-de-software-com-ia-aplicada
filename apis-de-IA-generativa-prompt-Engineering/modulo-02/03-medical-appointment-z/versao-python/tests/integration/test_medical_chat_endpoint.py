...
def test_cancel_not_found_is_structured(monkeypatch) -> None:
    # Cada cenário injeta seu catálogo para não depender do estado em memória
    # criado por testes anteriores da API.
    monkeypatch.setattr(app_module, "graph", build_graph(catalog=AppointmentCatalog(professionals=[Professional(1, "Dr. Alicio da Silva", "Cardiologia")])) )
...
def test_cancel_existing_appointment_returns_success(monkeypatch) -> None:
    ...
    monkeypatch.setattr(app_module, "graph", build_graph(fake, catalog))
